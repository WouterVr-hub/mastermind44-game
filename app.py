...
class GameState:
    ...
    def get_player_list_data(self):
        # THE FIX for 'undefined' names: Send a list of objects with a name property.
...
def handle_start_game():
    ...
    print("--- Starting Game with Server-Generated Secrets (Always 4 colors) ---")

    positions = list(range(1, CODE_LENGTH + 1))
    random.shuffle(positions)
    
    secrets_to_assign = []
    for _ in range(MAX_SECRET_HOLDERS):
        secrets_to_assign.append({ "pos": positions.pop(0), "color": random.choice(SECRET_COLORS) })

    for i, player_sid in enumerate(actual_players_sids):
        if i < len(secrets_to_assign):
            secret = secrets_to_assign[i]
            GAME.players[player_sid]["secret"] = secret
            emit('your_secret', secret, room=player_sid)
            print(f"Assigned secret to '{GAME.players[player_sid]['name']}': Pos {secret['pos']}, Color {secret['color']}")

    unassigned_position = positions[0]
    host_overview_data = {s['pos']: s['color'] for s in secrets_to_assign}
...
# No if __name__ block for Renderimport eventlet
eventlet.monkey_patch()
