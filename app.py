...
def handle_guess(data):
    ...
    # It does not give feedback per player anymore, but on the code as a whole.
    temp_secret = [c for c in GAME.secret_code if c != 'empty']
    ...
    for i in range(len(temp_guess)):
        if i < len(GAME.secret_code) and temp_guess[i] == GAME.secret_code[i] and temp_guess[i] != 'empty':
            black_pegs += 1
            # Mark as checked
            temp_guess[i] = None
            if GAME.secret_code[i] in temp_secret:
                temp_secret.remove(GAME.secret_code[i])
    ...
    # The feedback is now a simple list of 'black' and 'white' strings
    final_feedback = (['black'] * black_pegs) + (['white'] * white_pegs)
    
    GAME.guesses.append({"guesser": guesser_name, "guess": guess, "feedback": final_feedback})
    
    if data.get('is_final'):
        is_winner = black_pegs == NUM_COLOR_PEGS
...
# No if __name__ block for Render deploymentimport eventlet
eventlet.monkey_patch()
...
def handle_guess(data):
    ...
    for player_data in GAME.players.values():
        if "secret" in player_data:
            ...
            feedback[player_data["name"]] = {"black": black, "white": white, "giver": player_data["name"]}
    GAME.guesses.append({"guesser": guesser_name, "guess": guess, "feedback": feedback})
    
    if data.get('is_final'):
        num_secrets = sum(1 for p in GAME.players.values() if 'secret' in p)
        num_black_pegs = sum(1 for fb in feedback.values() if fb.get("black") == 1)
        is_winner = (num_secrets > 0 and num_black_pegs == num_secrets)
